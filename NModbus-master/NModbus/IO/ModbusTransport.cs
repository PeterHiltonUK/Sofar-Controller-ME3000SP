...
namespace NModbus.IO
{
    ...
    public abstract class ModbusTransport : IModbusTransport
    {
                    ...
                    lock (_syncLock)
                    {
                        ...
                        do
                        {
                            ...
                            if (message.V5Active)
                            {
                                ...
                                {
                                    case 3:
                                        response = WriteResponseV5<T>(message.VarName);
                                        break;
                                    default:
                                        response = ReadResponseV5<T>();
                                        break;
                                }
                            }
                            else
        ...
        public bool ShouldRetryResponse(IModbusMessage request, IModbusMessage response)
        {
            ...
            if (response is null || request.FunctionCode != response.FunctionCode)
            {
                return false;
            }
        ...
        public abstract IModbusMessage ReadResponseV5<T>()
            where T : IModbusMessage, new();

        public abstract IModbusMessage WriteResponseV5<T>(string name)
            where T : IModbusMessage, new();

        public abstract byte[] BuildMessageFrame(IModbusMessage message);

        public abstract void WriteV5Frame(IModbusMessage message);
